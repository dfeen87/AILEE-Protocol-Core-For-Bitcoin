...
protected:
    ...
    std::unique_ptr<IZKProvingBackend> active_backend;
...
public:
    ...
    IZKProvingBackend* get_backend_for_env(ZKBackendConfig& conf) {
        conf.type = select_backend_type(env, conf);
        if (env.is_ci && conf.type != ZKBackendType::MOCK) {
            throw DeterministicBackendException("Non-deterministic backend not allowed in CI");
        }
        active_backend = make_backend(conf);
...
TEST_F(OrchestrationIntegrationTest, ProofGeneratedWhenSchedulingSaysAttach) {
    ...
    auto* b = get_backend_for_env(config);

    auto result = orchestrate_epoch(ctx, b, config, &constraints, &transcript, state_root_hash);
...
TEST_F(OrchestrationIntegrationTest, MissingBackendReturnsEmptyMetadata) {
    auto ctx = get_context(AnchorDecision::ANCHOR, ProofDecision::ATTACH_PROOF);
    get_backend_for_env(config); // Test initialization
...
TEST_F(OrchestrationIntegrationTest, EmptyConstraintsEmptyTranscriptDeterministicHandling) {
    ...
    auto result = orchestrate_epoch(ctx, b, config, &empty_constraints, &empty_transcript, state_root_hash);
...
TEST_F(OrchestrationIntegrationTest, NullConstraintsHandledProperly) {
    ...
    auto result = orchestrate_epoch(ctx, b, config, nullptr, &transcript, state_root_hash);
...
TEST_F(OrchestrationIntegrationTest, NullTranscriptHandledProperly) {
    ...
    auto result = orchestrate_epoch(ctx, b, config, &constraints, nullptr, state_root_hash);
...
TEST_F(OrchestrationIntegrationTest, MismatchedIDsValidationFailure) {
    ...
    auto result = orchestrate_epoch(ctx, b, config, &mismatched_constraints, &mismatched_transcript, state_root_hash);
    auto& payload = result.anchor_payload;
...
TEST_F(OrchestrationIntegrationTest, NullStateRootHashHandledCorrectly) {
    ...
    auto result = orchestrate_epoch(ctx, b, config, &constraints, &transcript, "");
...
TEST_F(OrchestrationIntegrationTest, CIGuardThrowsOnNonDeterministicBackend) {
    env.is_ci = true;
    ZKBackendConfig halo2_config{ZKBackendType::HALO2_NATIVE, "test_circuit", "", "", ""};
    
    bool threw = false;
    try {
        get_backend_for_env(halo2_config);
    } catch (const DeterministicBackendException&) {
        threw = true;
    }
    EXPECT_TRUE(threw);
}

TEST_F(OrchestrationIntegrationTest, NonCIAllowsNonDeterministicBackend) {
    env.is_ci = false;
    ZKBackendConfig halo2_config{ZKBackendType::HALO2_NATIVE, "test_circuit", "", "", ""};
